...
def _load():
    ...
    if os.path.exists(DATA_FILE):
        try:
            ...
            log.info("Wczytano %d urządzeń", len(state["devices"]))
        except Exception as e:
            log.warning("Błąd odczytu: %s", e)
    if os.path.exists(WIZARD_FILE):
        try:
            with open(WIZARD_FILE) as f:
                state["wizard_devices"] = json.load(f)
            log.info("Wczytano %d urządzeń z wizarda", len(state["wizard_devices"]))
        except:
            pass
...
def _save():
    try:
        ...
        log.info("Zapisano %d urządzeń do %s", len(state["devices"]), DATA_FILE)
    except Exception as e:
        log.warning("Błąd zapisu: %s", e)
...
def run_scan(duration=None, force_cidrs=None):
    ...
    if force_cidrs:
        _log(f"Uruchamiam skan ({duration}s, force={','.join(force_cidrs)}) …", "info")
    else:
        _log(f"Uruchamiam skan ({duration}s) …", "info")
    ...
    try:
        ...
        _log(f"Zakończono — {len(scanned)} urządzeń (v3.3:{v33} v3.4:{v34})", "info")
    except Exception as e:
        _log(f"Błąd: {e}", "warn")
        log.exception("Szczegóły:")
    finally:
...
def run_wizard(api_region, api_key, api_secret):
    ...
    state["wizard_log"] = []
    _wlog("Uruchamiam tinytuya wizard …", "info")
    ...
    try:
            ...
            if os.path.exists(path):
                try:
                    ...
                    if isinstance(data, list):
                        devices = data
                        _wlog(f"Wczytano {len(devices)} urządzeń z {path}", "found")
                        break
                except:
                    pass
        ...
        if devices:
            ...
            try:
                ...
                pass
            _wlog(f"Gotowe — {len(devices)} urządzeń z kluczami!", "found")
            # Automatycznie uruchom skan po udanym wizardzie
            _wlog("Uruchamiam skan sieci...", "info")
            run_scan()
        else:
            _wlog("Wizard zakończony. Sprawdź logi powyżej.", "info")

    except Exception as e:
        _wlog(f"Błąd wizarda: {e}", "warn")
...
def api_device_dps(dev_id):
    try:
        ...
        if not dev:
            return jsonify({"ok": False, "msg": "Nie znaleziono urządzenia"}), 404
        if not dev.get("ip"):
            return jsonify({"ok": False, "msg": "Urządzenie nie jest online (brak IP)"}), 400
        if not dev.get("localKey"):
            return jsonify({"ok": False, "msg": "Brak Local Key dla urządzenia"}), 400
...
def api_wizard():
    ...
    if state["wizard_running"]:
        return jsonify({"ok": False, "msg": "Wizard już działa"}), 409
    data = request.json or {}
    region = data.get("region", "eu")
    key = data.get("apiKey", "")
    secret = data.get("apiSecret", "")
    if not key or not secret:
        return jsonify({"ok": False, "msg": "Wymagane: apiKey i apiSecret"}), 400
...
if __name__ == "__main__":
    ...
    log.info("Flask ruszył na porcie %d", PORT)
